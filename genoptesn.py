...
number_layers = reservoirconfig['number_layers']

if os.path.isdir(args.data):
    Xtr, Ytr, Xval, Yval, _, _, Yscaler = deepesn.load_from_dir(args.data, reconstructconfig)

elif dataType=='SantaFe' or dataType=='Sunspots' or dataType=='Lazy8X' or dataType=='Lazy8Y' or dataType=='GEFC' or dataType=='Mackey':
    #Xtr, Ytr, Xval, Yval, _, _, Yscaler = deepesn.generate_datasets_santafe(args.data)
    X, Y = deepesn.load_from_text(args.data)

    # Construct training/test sets
    Xtr, Ytr, Xval, Yval, _, _, Yscaler = deepesn.generate_datasets(X, Y, dataType = dataType)

    Xtr, Xval = deepesn.reconstruct_input_santafe([Xtr, Xval], reconstructconfig)
    Ytr, Yval = deepesn.reconstruct_output_santafe([Ytr, Yval], reconstructconfig)
    ...
    # Reconstruct
    Xtr, Xval = deepesn.reconstruct_input([Xtr, Xval], reconstructconfig)
    Ytr, Yval = deepesn.reconstruct_output([Ytr, Yval], reconstructconfig)
...
def gen_individual(prototype):
    ...
    def gen_val(dtype, minval, maxval):
        if dtype == 'f':
            ...
            value = []
            for i in range(number_layers):
                value.append(random.uniform(minval,maxval))
        elif dtype == 'sy':
            value = "Y"
        elif dtype == 'sn':
            value = "N"
...
def validate_individual(individual, prototype):
    ...
    for key in individual:
        ...
        # Check bounds
        if type(individual[key]) == list:
            for i in range(len(individual[key])):
                if individual[key][i] < minval:
                    individual[key][i] = minval
                elif individual[key][i] > maxval:
                    individual[key][i] = maxval
        elif type(individual[key]) == str:
            pass
        else:
...
def mutGaussianDict(individual, mu, sigma, indpb):
    ...
    for key in individual:
        if key == 'n_internal_units':
            if random.random() < indpb:
                if isinstance(sigma, dict):
                    for i in range(len(individual[key])):
                        individual[key][i] += int(random.gauss(mu, sigma[key]))
                else:
                    for i in range(len(individual[key])):
                        individual[key][i] += int(random.gauss(mu, sigma))
        elif type(individual[key]) == list:
            if random.random() < indpb:
                if isinstance(sigma, dict):
                    for i in range(len(individual[key])):
                        individual[key][i] += random.gauss(mu, sigma[key])
                else:
                    for i in range(len(individual[key])):
                        individual[key][i] += random.gauss(mu, sigma)
        elif type(individual[key]) == str:
            pass
        else:
            if random.random() < indpb:
...
def evaluate_ind(individual):
    ...
    for i in range(n_eval):
        _, errors[i] = deepesn.run_from_config(Xtr, Ytr, Xval, Yval, parameters, reservoirconfig, Yscaler, dataType)
    ...
    if parameters['n_dim'] is None:
        ...
        if type(parameters['n_internal_units'])==list:
            return error,float(parameters['n_dim'])/parameters['n_internal_units'][-1]
        else:
